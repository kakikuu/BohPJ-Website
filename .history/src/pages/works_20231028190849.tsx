    ...
    <main>
      ...
      <div>
        ...
        <div className={styles.WorksContens}>
          <h2>Works</h2>
          {assets.map((a) => {
            return <div className={styles.WorkDisplay}>
              <iframe src={`https:${a.file.url}`} />
            </div>
          })}
          {posts.map((p) => {
            return <div>
              <h2 className={styles.WorksContens}>{p.title}</h2>
              <p className={styles.ContentsTitle.p}> {p.workoverview}</p>
            </div>
          })}
        </div>
      </div>
